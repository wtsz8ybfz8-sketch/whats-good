...
| `osmFallback.ts` — occasion narrowing | 2026-08-15 | Chromium 390×844, Places forced to 403, Overpass answered by a hand-written fixture | **PARTIALLY EXERCISED — WITH A FIXTURE, NEVER AGAINST REAL OVERPASS.** `narrowByQuery` measured across three occasions: "Coffee & a laptop" → 1 (matched `amenity=cafe`, since `cuisine=coffee_shop` shares no substring with the query word `cafe`), "Braai" → 1 (`cuisine=barbecue`), "Quick lunch" → all 5 (no food word matched; the documented keep-everything branch). Before this change all three returned the identical list. **NOT checked: the real OSM tag vocabulary — `wiki.openstreetmap.org` is EGRESS_BLOCKED from this container, so the fixture's tag values are my own and the match rate against real Overpass data is UNKNOWN. Also not checked: `NOT_A_CUISINE` against real user phrasing, and whether narrowing to 1 result is too aggressive on a real city's data.** |
| `osmFallback.ts` — everything else | 2026-08-14 | Source trace only | **NEVER EXERCISED.** Declared 2026-08-14. The Overpass path **cannot be run from this container** — egress is blocked (`EGRESS_BLOCKED` from the fetch tool, `000` from curl to all three mirrors). Its opening-hours parser, its mirror failover and its `openNow` derivation have never produced a value that anyone has looked at. Verify from a real device or from Vercel runtime logs. |
| `api/recipes.ts` | **NEVER INSPECTED** | — | Serves the Cook tab from a hand-curated corpus of schema.org Recipe JSON-LD pages, keyed on the six effort occasions (`quick30`, `onepot`, `pantry`, `veg`, `batch`, `baking`) and answering 400 for any other key. A Vercel function, so it is **outside `tsconfig` and no local gate covers it**. Declared here 2026-08-29 after CI caught it missing. Read in source that day to confirm the occasion keys before the Cook tiles were nearly renamed — that rename would have 400'd every recipe request. Never invoked locally: `vite dev` does not serve `/api`. |
